...
googleMaps = {
  ...
  createAircraftMarker: (position, name, hide, clickEvent) => {
    ...
    aircraftMarker.addListener("click", (event) => {
      var items = googleMaps.getItemsInCircle(
        googleMaps.getPixelPosition(event.latLng),
        32
      );
  ...
  getMarkerIcon: (color, clicked, aerobatic, label, point) => {
    ...
    if (!clicked) {
      iconUrl = googleMaps.getMarkerIconUrl(
        color,
        false,
        aerobatic,
        label,
  ...
  getItemsInCircle: (pixel, radius) => {
    ...
    var aircraftsInCircle = $.map(aircrafts, (aircraft, index) => {
      ...
      if (
        aircraftMarker.getVisible() &&
        aircraftMarker.map != null &&
        googleMaps.distanceBetweenPixels(pixel, aircraftPixel) < radius
      ) {
        return [aircraft];
      } else {
        return [];
    ...
    var locationsInCircle = $.map(locations, (location, index) => {
      if (
        location !== undefined &&
        markersMap[location.pointId] !== undefined
      ) {
  ...
  getMarkerIconUrl: (color, clicked, aerobatic, label, point) => {
    ...
    color = color.replace("#000000", "00000000");
  ...
  drawRouteOnMap: (route) => {
    ...
    route.points.forEach((point) => {
      if (!point.hidden) {
        ...
        var markerHtml = googleMaps.getMarkerIcon(
          route.color,
          false,
          aerobatic,
          point.pointName,
        ...
        var markerHtmlClicked = googleMaps.getMarkerIcon(
          route.color,
          true,
          aerobatic,
          point.pointName,
              ...
              if (selectedLocation != null) {
                deselectLocation(() => {
                  ...
                  selectLocation(
                    ...
                    markerHtml,
                    markerHtmlClicked,
                    "#" + route.color.toLowerCase(),
                    "#" + route.primaryTextColor,
                    "#" + route.secondaryTextColor
                  );
                });
              } else {
                // then show a new popup
        ...
        if (!isPointAerobatic(point.pointId)) allMarkers.push(marker);
