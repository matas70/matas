...
function initPopups() {
  ...
  let topY;
  let finalTouchY;
  let fullWindowHeight = $(window).height();
  -$("headerBg").height();
...
function showBaseLoactionPopup(pointId) {
  ...
  document.getElementById("waze-base-link").href =
    point.baseWazeDestinationLink;
  ...
  document.getElementById("baseArrivalExplanation").innerHTML =
    point.baseArrivalExplanation;
...
function showLocationPopup(
  ...
  point.aircrafts.forEach((ac) => {
    if (ac.specialInAircraft) {
      if (
        !specials.has(ac.specialInAircraft) &&
        getCurrentTime() < getActualPathTime(ac.date, ac.time)
      ) {
  ...
  if (point.pointLocation) $("#popupSubTitle").text(point.pointLocation);
  else $("#popupSubTitle").text("");
  ...
  if (point.pointName === "ירושלים - גן סאקר" && point.aircrafts.length > 0) {
    let endTime = createAerobaticEndTime(
      point.aircrafts[point.aircrafts.length - 1].time,
      10
    );
    $("#popupTime").text(
      roundToMinute(point.aircrafts[0].time) + "-" + endTime
    );
  } else if (
    point.aircrafts.length === 1 &&
    point.aircrafts[0].name === "עפרוני" &&
    point.pointName.includes("מופע")
  ) {
...
function fillAircraftSchedule(aircraft, showAllPoints = false) {
  ...
  if (showAllPoints) {
    ...
    aircraftPath = removeDuplicatesBy(
      (point) => point.pointId,
      [].concat.apply(
        [],
        aircrafts
      ...
      locations[point1.pointId].pointName.localeCompare(
        locations[point2.pointId].pointName
      )
    );
  } else {
...
function showAircraftInfoPopup(aircraft, collapse) {
  $("#cockpitVideo").hide();
  if (aircraft.icon === "efroni") $("#cockpitVideo").show();
  ...
  var startTime = aircraft.path[0].from
    ? aircraft.path[0].from
    : aircraft.path[0].time;
  var endTime = aircraft.path[0].from
  ...
  $("#aircraftInfoIcon").attr(
    "src",
    "icons/aircraft-menu/" + aircraft.icon + ".svg"
  );
  ...
  let currAircraftName = aircraftUsdzObjData.find(
    (obj) => obj.hebName === aircraft.name
  );

  if (currAircraftName !== undefined) {
    document.getElementById("aircraftInfo3D").innerHTML =
      "<button id='ARButton' onclick='openAR(" +
      JSON.stringify(currAircraftName) +
      ")'></button>";
  } else {
    document.getElementById("aircraftInfo3D").innerHTML =
...
function createParachutistRow(location, time, from) {
  return (
    '<div onclick="selectPointFromSchedule(' +
    location.pointId +
    ')" class="tableRow aerobatic"><img src="icons/aircraft-menu/parachutist.svg" class="parachutistIcon"></img> <div class="aircraftName"><b>' +
    location.pointName +
    '</b></div><div class="time">' +
    roundToMinute(time) +
...
function createAerobaticRow(location, time, from) {
  return (
    '<div onclick="selectPointFromSchedule(' +
    location.pointId +
    ')" class="tableRow aerobatic"><img src="icons/aircraft-menu/aerobatic.svg" class="aerobaticIcon"></img> <div class="aircraftName"><b>' +
    location.pointName +
    '</b></div><div class="time">' +
    roundToMinute(time) +
    (from ? " - " + roundToMinute(from) : "") +
...
function createCategoryLocationRow(location, time, from) {
  return (
    '<div onclick="selectPointFromSchedule(' +
    location.pointId +
    ')" class="tableRow indented"><div class="aircraftName"><b>' +
    location.pointName +
    '</b></div><div class="time">' +
    (from ? roundToMinute(from) + " - " : "") +
    roundToMinute(time) +
...
function createScheduleRow(aircraft, location) {
  ...
  if (!fullPoint.hidden || isNotHidden(fullPoint)) {
    if (aircraft.aerobatic) {
      ...
      return createLocationScheduleRow(
        aircraft,
        fullPoint,
        location.time,
        location.from
...
function createTableRow(
  ...
  if (
    ...
    classname =
      '<button class="show" id="ARButton" onclick=\'openAR(' +
      JSON.stringify(currAircraftName) +
      ")'></button>";
  } else
    classname =
      '<button class="hide" style="display: none;" id="ARButton" onclick=\'openAR(' +
      JSON.stringify(currAircraftName) +
      ")'></button>";
  if (aerobatic) {
    aerobaticIcon =
      '<img src="icons/aircraft-menu/aerobatic.svg" class="aerobaticTableIcon"></img>';
    //aircraftType = "מופע אווירובטי";
  } else if (special === "הצנחת צנחנים") {
    aerobaticIcon =
      '<img src="icons/aircraft-menu/parachutist.svg" class="aerobaticTableIcon"></img>';
    aircraftType = "הצנחת צנחנים";
  }
  return (
    "<div onclick='onAircraftSelected(" +
    aircraftId +
    "," +
    collapse.toString() +
    "," +
    ...
    "," +
    showAllPoints +
    ');\' class="tableRow"><img src="icons/aircraft-menu/' +
    icon +
    '.svg" class="aircraftIcon"><div class="aircraftName"><b>' +
    name +
    "</b> " +
    aircraftType +
    "</div>" +
    ...
    classname +
    '<div class="time">' +
    (displayTime ? roundToMinute(time) : "") +
    (displayTime && from ? " - " + roundToMinute(from) : "") +
    "</div></div></div></div>"
  );
}
...
function createLocationRow(
  ...
  if (
    location.aircrafts.length == 0 &&
    !location.pointName.includes("תצפית למטס")
  )
    return "";

  return (
    "<a class='locationRow' href='javascript:void(0);'><div id='location" +
    location.pointId +
    "' class='locationRow' onclick='expandLocation(" +
    location.pointId +
    "," +
    isSearchBar +
    ");'>" +
    ...
    "</div>" +
    "</div></a>" +
    "<div id='locationSpace" +
    location.pointId +
    "' class='locationSpace'></div>" +
...
function expandLocation(pointId, isSearchBar = false) {
  ...
  var locationSpace = isSearchBar
    ? $("#search-view #locationSpace" + pointId)
    : $("#locationSpace" + pointId);
  if (locationSpace.html() === "") {
    ...
    $("#location" + pointId)
      .children(".nextAircraftSection")
      .children(".expandArrow")
      .hide();
    $("#location" + pointId)
      .children(".nextAircraftSection")
...
function showBasePopup(
  ...
  if (
    isAerobatics ||
    special1 === "מופעים אוויריים" ||
    special2 === "מופעים אוויריים"
  ) {
...
function createClusterLocationRow(location) {
  ...
  if (
    ...
    (location.pointName.includes("בסיס") &&
      !location.pointName.includes("חצור"))
  ) {
    resultLocationRow =
      `<div onclick='selectPoint(` +
      location.pointId +
      ');\' class="tableRow"><img src="icons/OpenBaseMap.svg " class="locationIconForBases"><div class="aircraftName"><b>' +
      location.pointName +
      "</b></div></div></div>";
  } else {
    resultLocationRow =
      `<div onclick='selectPoint(` +
      location.pointId +
      ');\' class="tableRow"><img src="icons/point-' +
      location.color +
      '.svg " class="locationIcon"><div class="aircraftName"><b>' +
      location.pointName +
      "</b></div></div></div>";
  }
...
function openMapClusterPopup(arrayOfObjects) {
  ...
  $("#mapClusterPopup").css("z-index", "1000");
