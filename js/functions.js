...
function loadOpenBasesLocation(callback) {
  getEnv((env) => {
    $.getJSON(
      `${config.apiURL}/${env}/openBases.json?t=` + new Date().getTime(),
...
function getPathLocation(pointId) {
  ...
  else return convertLocation(loc.N, loc.E);
}
...
function getCurrentIndexLocation(path, currentTime) {
  ...
  if (
    plannedStartTime + relativeTime <
    convertTime(path[prevLocation].date, path[prevLocation].time)
  )
    return -1;

  // otherwise - search for the two points where the aircraft suppossed to be between
  while (nextLocation < path.length && !found) {
    var nextLocationTime = convertTime(
      path[nextLocation].date,
      path[nextLocation].time
    );
...
function getCurrentLocation(path, currentTime) {
  ...
  var prevLocationTime = convertTime(
    path[prevLocation].date,
    path[prevLocation].time
  );
  var ratio =
    (plannedStartTime + relativeTime - prevLocationTime) /
    (nextLocationTime - prevLocationTime);

  return getRelativeLocation(
    getPathLocation(path[prevLocation].pointId),
    getPathLocation(path[nextLocation].pointId),
    ratio
  );
}
...
function getNextLocation(path, currentTime) {
  if (path.length > 1) {
    ...
    if (nextLocation === -1) {
      var nextTime =
        convertTime(path[1].date, path[1].time) -
        plannedStartTime +
        actualStartTime;
      return { location: getPathLocation(path[1].pointId), time: nextTime };
    } else if (nextLocation == path.length) {
      return {
        location: getPathLocation(path[path.length - 1].pointId),
        time: currentTime,
      };
    } else {
      var nextTime =
        convertTime(path[nextLocation].date, path[nextLocation].time) -
        plannedStartTime +
        actualStartTime;
      return {
        location: getPathLocation(path[nextLocation].pointId),
        time: nextTime,
      };
    }
  } else {
  ...
  if (!userSimulation) {
    pathPassed.forEach(function (path) {
      ...
      location.aircrafts = location.aircrafts.filter(function (aircraftInPath) {
        return (
          aircraftInPath.aircraftId !== aircraft.aircraftId ||
          (aircraftInPath.aircraftId === aircraft.aircraftId &&
            currTime <
              getActualPathTime(aircraftInPath.date, aircraftInPath.time))
        );
      });
    });
  }
...
function containsPosition(pos, list) {
  ...
  for (i = 0; i < list.length; i++) {
    if (list[i].lat === pos.lat && list[i].lng == pos.lng) {
      return true;
    }
  }
...
function indexOfPosition(pos, list) {
  var i;
  for (i = 0; i < list.length; i++) {
    if (list[i].position.lat === pos.lat && list[i].position.lng == pos.lng) {
      return i;
    }
  }
...
function getHtmlWithAerobaticGlow(originalMarkerHtml) {
  if (!originalMarkerHtml.includes("aerobatic-gif")) {
    ...
    var htmlWithGif = originalMarkerHtml.replace(
      markerClassHtml,
      markerClassHtml + " aerobatic-gif-marker"
    );
...
function glowOnPoint(location, timeOfAerobaticShow) {
  if ($.urlParam("ff") === "true")
    timeOfAerobaticShow = timeOfAerobaticShow / 60;

  var relevantMarker = markersMap[location.pointId];

  if (relevantMarker) {
    const originalMarkerHtml = mapAPI.getMarkerHtml(relevantMarker);
    mapAPI.setMarkerHtml(
      relevantMarker,
      getHtmlWithAerobaticGlow(originalMarkerHtml)
    );

    // Actually set the icon
    mapAPI.setMarkerIcon(
      relevantMarker,
      mapAPI.getMarkerIconToSet(relevantMarker)
    );
...
function scheduleAerobaticNotifications(
  notificationBody,
  item,
  location,
  time
  ...
  aircrafts.forEach(function (aircraft) {
    ...
    aircraft.path.forEach(function (location) {
      ...
      if (locations[location.pointId]) {
        ...
        console.warn(
          `warning - aircraft is moving above non existing location, point id: ${location.pointId}, time: ${item.time}`
        );
        aircraft.path = aircraft.path.filter(
          (point) => point.pointId !== location.pointId
        );
        location.aircrafts = [];
      ...
      if (
        displayAircraftShows &&
        (item.aerobatic ||
          item.parachutist ||
          item.specialInPath === "מופעים אוויריים" ||
          item.specialInAircraft === "מופעים אוויריים")
      ) {
        var timeout =
          convertTime(item.date, item.time) -
          getCurrentTime() +
          actualStartTime -
          plannedStartTime;
        var notificationBody = `${getEventName(
          item.aerobatic,
          item.specialInAircraft,
          item.specialInPath
        )} ${getEventDescription(item.aerobatic, location.pointName, 5)}`;
        if (!userSimulation && timeout > 0) {
          scheduleAerobaticNotifications(
            notificationBody,
            item,
            location,
            timeout
...
function loadLocations(callback) {
  getEnv((env) => {
    $.getJSON(
      `${config.apiURL}/${env}/points.json?t=` + new Date().getTime(),
      function (points) {
...
function loadRoutes(callback) {
  getEnv((env) => {
    $.getJSON(
      `${config.apiURL}/${env}/routes.json?t=` + new Date().getTime(),
      function (routes) {
...
function loadActualStartTime() {
  ...
  if ($.urlParam("simulation") != null) {
    actualStartTime =
      new Date().getTime() - $.urlParam("simulation") * 60 * 1000;
  }
  ...
  rehearsals.forEach((rehersal) => {
    if (
      currentTime >
        convertTime(rehersal.date, rehersal.time) - deltaFromRehearsals &&
      currentTime <
        convertTime(rehersal.date, rehersal.time) + deltaFromRehearsals
    )
  ...
  if (
    actualStartTime - currentTime > 6 * 60 * 60 * 1000 &&
    !isRehearsalActive
  ) {
    ...
    var simulationLength =
      lastFlightTime - 15 * 60 * 1000 - (firstFlightTime + 15 * 60 * 1000);
    actualStartTime =
      currentTime -
      (currentTime % ((firstFlightTime + 15 * 60 * 1000) % simulationLength)) -
      (firstFlightTime + 15 * 60 * 1000 - actualStartTime);
  }
}
...
function loadAircrafts(callback) {
  getEnv((env) => {
    $.getJSON(
      `${config.apiURL}/${env}/aircrafts-info.json?t=` + new Date().getTime(),
      function (aircraftInfo) {
        ...
        // load all aircrafts
        $.getJSON(
          `${config.apiURL}/${env}/aircrafts.json?t=` + new Date().getTime(),
          function (flightData) {
            aircrafts = flightData.aircrafts;
            startDate = flightData.startDate;
            plannedStartTime = convertTime(
              startDate,
              flightData.plannedStartTime
            );
            ...
            aircrafts = aircrafts.filter(
              (aircraft) => aircraft.path.length > 1
            );
            ...
            aircrafts.forEach(function (aircraft) {
              if (aircraft.aircraftTypeId !== undefined) {
                ...
                var aircraftTypeInfo =
                  aircraftTypesInfo[aircraft.aircraftTypeId];
              ...
              aircraft.path.sort(
                (point1, point2) =>
                  convertTime(point1.date, point1.time) -
                  convertTime(point2.date, point2.time)
              );

              // update times of all flights
              if (
                !aircraft.hide &&
                aircraft.path.length > 0 &&
                !aircraft.special
              ) {
                aircraftFlightTime = convertTime(
                  aircraft.path[0].date,
                ...
                aircraftLandTime = convertTime(
                  aircraft.path[aircraft.path.length - 1].date,
                  aircraft.path[aircraft.path.length - 1].time
                );
...
function shouldShowTypeCategory(typeCategory) {
  return !!locations.find(
    (location) => location && location.type === typeCategory
  );
}
    ...
    navigator.geolocation.getCurrentPosition(
      function (position) {
        ...
        gtag("event", "show_curr_location", {
          ...
          event_label:
            "success - show point " + findClosestPoint(currentPosition),
        });
      },
      function () {
  ...
  if (userSimulation) {
    ...
    let remainingAircrafts = aircrafts.filter((aircraft) => {
      let path = aircraft.path.filter((point) => {
        return getActualPathTime(point.date, point.time) > cursrentTime;
      });
...
function animateToNextLocation(aircraft, previousAzimuth, updateCurrent) {
  ...
  if (
    ...
    !notifiedNearUser.includes(aircraft.aircraftTypeId)
  ) {
    var userLoc = null;
  ...
  if (
    ...
    mapAPI.toggleAircraftMarkerVisibility(
      aircraftMarkers[aircraft.aircraftId],
      false
    );
    ...
    if (nextAircraftStopPosition.time - currentTime > animationTime) {
      nextAircraftPosition = getPredictedLocation(
        currentAircraftPosition,
        nextAircraftStopPosition.location,
        nextAircraftStopPosition.time - currentTime,
        animationTime
      );
    } else {
    ...
    var currentAircraftAzimuth = calcAzimuth(
      currentAircraftPosition,
      nextAircraftPosition
    );
    ...
    if (
      curIndexLocation >= 0 &&
      curIndexLocation < aircraft.path.length &&
      (aircraft.path[curIndexLocation].hideAircrafts || aircraft.hide)
    ) {
...
function startAircraftsAnimation(updateCurrent) {
  aircrafts.forEach(function (aircraft) {
    ...
    if (
      convertTime(aircraft.path[0].date, aircraft.path[0].time) -
        plannedStartTime +
        actualStartTime >
      getCurrentTime()
    ) {
      ...
      animateToNextLocation(
        aircraft,
        aircraftMarkers[aircraft.aircraftId].currentAircraftAzimuth,
        updateCurrent
      );
    }
  }, this);
...
function checkDeparture(aircraft) {
  if (
    ...
    if (
      nextLocation >= 0 &&
      nextLocation < aircraft.path.length &&
      aircraft.path[nextLocation].hideAircrafts
    ) {
      mapAPI.toggleAircraftMarkerVisibility(
        aircraftMarkers[aircraft.aircraftId],
        false
...
function addAircraftsToMap() {
  ...
  aircrafts.forEach(function (aircraft) {
    ...
    var currentAircraftPosition = getCurrentLocation(
      aircraft.path,
      getCurrentTime()
    );
    var nextAircraftPosition = getNextLocation(aircraft.path, getCurrentTime());
    var currentAircraftAzimuth =
      calcAzimuth(currentAircraftPosition, nextAircraftPosition.location) % 360;
    aircraft.currentAircraftAzimuth = currentAircraftAzimuth;
        ...
        if (selectedAircraft != null) {
          deselectAircraft(function () {
            ...
            selectAircraft(
              ...
              aircraft.icon,
              aircraft.image,
              roundToMinute(aircraft.path[0].time),
              aircraft.infoUrl,
              false
            );
          });
    ...
    var aircraftMarker = mapAPI.createAircraftMarker(
      currentAircraftPosition,
      aircraft.name,
      aircraft.hide,
      clickCallback
...
function updateAircraftIcons() {
  ...
  aircrafts.forEach(function (aircraft) {
    ...
    setAircraftIcon(
      ...
      aircraft.aircraftId,
      aircraft.country,
      aircraft.currentAircraftAzimuth,
      aircraft.color,
      zoomLevel
    );
  }, this);
...
function selectLocation(
  ...
  if (aerobaticShows[selectedPointId]) {
    markerIconClicked = mapAPI.setMarkerIconHtml(
      markerIconClicked,
      getHtmlWithAerobaticGlow(mapAPI.getMarkerIconHtml(markerIconClicked))
    );
    marker = mapAPI.setMarkerHtml(
      marker,
      getHtmlWithAerobaticGlow(mapAPI.getMarkerHtml(marker))
    );
  }
  ...
  if (
    locations[pointId].pointName.includes("בסיס") &&
    !locations[pointId].pointName.includes("חצור")
  ) {
...
function openAR(aircraft) {
  if (isIOS()) {
    gtag("event", "showAircraftIOS", {
      ...
      event_label:
        localStorage.getItem("selectedAircraftIsUsdz") || "default aircraft",
    });
    ...
    if (!document.querySelector("#popup-bottom #checkbox").checked) {
      ...
      document
        .querySelector("#popup-bottom button")
        .addEventListener("click", () => {
          ...
          if (localStorage.getItem("selectedAircraftIsUsdz")) {
            openExternal(
              `https://matasstorage.blob.core.windows.net/models/usdz%2F${localStorage.getItem(
                "selectedAircraftName"
              )}.usdz`
            );
          } else {
            openExternal(
              `https://matasstorage.blob.core.windows.net/models/usdz%2Fbarak.usdz`
            );
          }
        });
    } else {
...
function setMarkerOnDeselectLocation() {
  // set it to the previous marker icon
  if (aerobaticShows[selectedPointId]) {
    selectedLocationMarker = mapAPI.setMarkerHtml(
      selectedLocationMarker,
      getHtmlWithAerobaticGlow(mapAPI.getMarkerHtml(selectedLocationMarker))
    );
    selectedLocationMarkerIcon = mapAPI.setMarkerIconHtml(
      selectedLocationMarkerIcon,
      getHtmlWithAerobaticGlow(
        mapAPI.getMarkerIconHtml(selectedLocationMarkerIcon)
      )
    );
  }
...
function selectPoint(pointId, minimized = false) {
  ...
  var selectedRoute = routes.find((route) =>
    route.points.includes(selectedPoint)
  );
  ...
  if (selectedLocation != null) {
    deselectLocation(function () {
      ...
      selectLocation(
        pointId,
        convertLocation(selectedPoint.N, selectedPoint.E),
        marker,
        mapAPI.getMarkerIcon(
          selectedRoute.color,
          false,
          isPointAerobatic(pointId),
        ...
        "#" + selectedRoute.color,
        "#" + selectedRoute.primaryTextColor,
        "#" + selectedRoute.secondaryTextColor,
        minimized
      );
    });
  } else {
...
function onHomeButtonClick() {
  ...
  if (mapLoaded) {
    if (!currentLocationMarker) {
      mapAPI.focusOnLocation({ lat: 32.0, lng: 35.0 }, 8);
      showCurrentLocation();
    } else {
      selectPoint(
        findClosestPoint(mapAPI.getMarkerPosition(currentLocationMarker)),
        true
      );
    }
  }
...
function countdown() {
  ...
  if (remainingTime < 0) {
    ...
    $("#days").text(
      getRemainingDays(remainingTime) < 1
        ? getRemainingHours(remainingTime) < 10
          ? "0" + getRemainingHours(remainingTime)
          : getRemainingHours(remainingTime)
        : getRemainingDays(remainingTime) < 10
        ? "0" + getRemainingDays(remainingTime)
        : getRemainingDays(remainingTime)
    );
    $("#hours").text(
      getRemainingDays(remainingTime) < 1
        ? getRemainingMinutes(remainingTime) < 10
          ? "0" + getRemainingMinutes(remainingTime)
    ...
    $("#minutes").text(
      getRemainingDays(remainingTime) < 1
        ? getRemainingSeconds(remainingTime) < 10
          ? "0" + getRemainingSeconds(remainingTime)
...
function getRemainingHours(date) {
  return (
    Math.floor(date.getTime() / (1000 * 60 * 60)) - 24 * getRemainingDays(date)
  );
}
...
function getRemainingSeconds(date) {
  return Math.round(
    (date.getTime() -
      ((getRemainingDays(date) * 24 + getRemainingHours(date)) * 60 +
        getRemainingMinutes(date)) *
        60 *
        1000) /
      1000
  );
    ...
    if (!($.urlParam("offline") === "true")) {
      setTimeout(() => {
        ...
        if (!appLoaded && navigator.serviceWorker) {
          console.log(
            "app load is taking too long... trying to switch to offline mode."
          );
...
function loadMapApi() {
  ...
  if (!mapLoaded) {
    if ($.urlParam("offline") === "true") {
      ...
      fetch("data/test-connection.json?t=" + new Date().getTime())
        .then((response) => {
  ...
  if (!searchOpen) {
    ...
    if (shouldShowTypeCategory("base")) {
      ...
      sortedLocations.forEach(function (location) {
        if (
          (location.pointName.includes("בסיס") &&
            !location.pointName.includes("בסיס חצור")) ||
          location.pointName.includes("מוזיאון חיל האוויר")
        ) {
    ...
    sortedLocations.forEach(function (location) {
      if (
        ((!location.hidden &&
          !!routes.find((route) =>
            route.points.find((point) => location.pointId === point.pointId)
          )) ||
          isNotHidden(location)) &&
        (location.type !== "base" ||
          (!location.pointName.includes("בסיס") &&
            location.pointName !== "בסיס חצור") ||
          !location.pointName.includes("מוזיאון ") ||
          location.type !== "hospital" ||
          !location.pointName.includes("תצפית למטס"))
      ) {
        searchViewHtml += createLocationRow(location, false, true);
...
function search_GA_report(needle) {
  if (search_GA_report_timeout) {
    clearTimeout(search_GA_report_timeout);
  }
  ...
  $(".search-input").keyup(function () {
    ...
    basesResults = sortedLocations.filter((location) => {
      return (
        (location.pointName.includes("בסיס") ||
          location.pointName.includes("מוזיאון חיל האוויר")) &&
        location.pointName !== "בסיס חצור" &&
        location.pointName.includes(searchInput)
      );
    });
    ...
    if (basesResults.length > 0) {
      ...
      basesResults.forEach(function (location) {
        resultsHtml += createLocationRow(location, false, true);
      }, this);
    }
    ...
    citiesResults = sortedLocations.filter((location) => {
      return (
        (!location.hidden || isNotHidden(location)) &&
        ((location.type !== "base" &&
          location.type !== "hospital" &&
          !location.pointName.includes("מוזיאון") &&
          !location.pointName.includes("בסיס") &&
    ...
    aircraftResults = Array.from(aircraftMap.values()).filter(
      (aircraft) =>
        aircraft.name.includes(searchInput) ||
    ...
    if (
      ...
      basesResults.length > 0 ||
      viewPointResults.length > 0
    ) {
      $("#search-prompt").hide();
      $("#search-view").show();
...
window.onhashchange = (e) => {
  ...
  // Should close the menu
    ...
    (previousHashValue === menuHash || previousHashValue === locationsHash) &&
    (currentHash === mainHash || currentHash === "/")
  ) {
    $("#menuHamburger").click();
  } else if (
    previousHashValue === locationsHash &&
    currentHash === aircraftHash
  ) {
    // Should toggle between locations and aircraft
    $("#aircraftLink").click();
  } else if (
    previousHashValue === aircraftHash &&
    (currentHash === locationsHash || currentHash === menuHash)
  ) {
    ...
    (previousHashValue === aircraftSelectedHash ||
      previousHashValue === aircraftInfoContentHash) &&
    ...
    previousHashValue === aircraftScheduleContentHash &&
    (currentHash === aircraftInfoContentHash ||
      currentHash === aircraftSelectedHash ||
      currentHash === moreInfoHash)
  ) {
    $("#aircraftInfoButton").click();
  } else if (
    previousHashValue === moreInfoHash &&
    currentHash !== moreInfoHash
  ) {
    $("#shrinkAircraftInfoPopup").click();
  }
  ...
  // Location popup
    ...
    previousHashValue === locationPopupHash &&
    currentHash !== locationPopupHash
  ) {
    closeAllPopups();
  }
};
...
function loadCategories(callback) {
  getEnv((env) => {
    $.getJSON(
      `${config.apiURL}/${env}/categories.json?t=` + new Date().getTime(),
...
function fillMenu() {
  ...
  categories.forEach(function (category) {
    ...
    if (category.special) {
      ...
      var categoryAircrafts = categorizedAircrafts
        .filter((aircraft) => aircraft.special === category.category)
        .sort((aircraft1, aircraft2) => {
          return aircraft1.name > aircraft2.name
            ? 1
            : aircraft1.name < aircraft2.name
            ? -1
            : 0;
        })
        .filter((categoryAircraft) =>
          categoryAircraft.path.find(
            (point) => getCurrentTime() <= convertTime(point.date, point.time)
          )
        );
      ...
      aircraftsForCategory = Array.from(aircraftMap.values())
        .filter((aircraft) => aircraft.category === category.category)
        .sort((aircraft1, aircraft2) => {
        ...
        .filter((categoryAircraft) =>
          categoryAircraft.path.find(
            (point) =>
              (point.date && new Date(point.date) > new Date()) || !point.date
          )
        );
      ...
      if (aircraftsForCategory.length > 0) {
        ...
        aircraftsForCategory.forEach(function (aircraftFromCategory) {
          html += createTableRow(
            aircraftFromCategory.aircraftId,
...
function scheduleConfirmationPopup() {
  let messageBody =
    "אם ברצונך לקבל הודעה בדבר זמני המופעים הקרובים עליך לאשר את ההתראות";
...
function initGenericPopups() {
  if (userSimulation) {
    showGenericPopup(
      "מחממים מנועים!",
      "המטוסים המופיעים על המפה לפני המטס הינם הדמייה בלבד",
      undefined,
      null,
      () => {
        setTimeout(() => {
          showGenericPopup(
            "בזמן שאתם ממתינים...",
            "הבאנו את מטוסי החיל אליכם,<br>בואו לצפות במטוסים אצלכם בסלון",
            "arIcon",
            "ar.html"
          );
        }, 10 * 1000);
    ...
    getCurrentTime() >= realActualStartTime - 4 * 60 * 60 * 1000 &&
    getCurrentTime() <= realActualStartTime + 3 * 60 * 60 * 1000
  ) {
    if (!changes) {
      let displayed = "false";
      if (localStorage)
      ...
      if (!(displayed === "true")) {
        showGenericPopup(
          "בוקר כחול לבן!",
          `השמיים מושלמים למטס. <br> בואו לחגוג איתנו :)`,
          "flightStartIcon",
          () => {
            setTimeout(() => {
              showGenericPopup(
      ...
      showGenericPopup(
        "עקב תנאי מזג האוויר",
        `חלו שינויים קלים בנתיבים ובמופעים, אך אנחנו עדיין באים! (: חג שמח!`,
    ...
    var timeToFlightEnd =
      new Date(realActualStartTime).addHours(6) - new Date();
    ...
    setTimeout(() => {
      showGenericPopup(
        "נתראה בשנה הבאה!",
        `מקווים שנהניתם מהמטס. <br> חג עצמאות שמח! :)`,
        "flightEndIcon"
      );
    }, timeToFlightEnd);
  }

  var timeToNotifyOfek =
    new Date(realActualStartTime).addHours(2.5) - new Date();
          ...
          if ($("#map:visible").length === 1) {
            ...
            aircrafts.forEach((aircraft) => {
              if (!loadedAicrafts[aircraft.icon]) {
                $("#aircraftImgCache").append(
                  `<img id="${aircraft.icon}" src="icons/aircrafts/${aircraft.icon}.svg" class="imageCache"></img>`
                );
...
function getAerobaticsPoints() {
  return [].concat(
    aircrafts
      .filter((aircraft) => aircraft.aerobatic)
      .map((aircraftObj) => aircraftObj.path.map((point) => point.pointId))
      .flat(),
    aircrafts
      .map((aircraft) =>
        aircraft.path
          .filter((point) => point.special === "מופעים אוויריים")
...
function getAllPointsWithShows() {
  if (!pointsWithShows) {
    pointsWithShows = [].concat.apply(
      [],
      aircrafts
        .filter((aircraft) => {
          return (
            aircraft.aerobatic ||
            aircraft.specialInPath ||
...
function isAircraftAerobatic(aircraftId) {
  return (
    aircrafts[aircraftId].aerobatic ||
    aircrafts[aircraftId].specialInPath === "מופעים אוויריים"
  );
}

function getEventName(aerobatic, special1, special2) {
  if (aerobatic) return "מופע אווירובטי";
  else if (special1 === "מופעים אוויריים") return "מופע אווירי";
  else if (special2 === "מופעים אוויריים") return "מופע אווירי";
  else {
    return "מופע";
  }
}
...
function notifyUserIfNear(currentLocation, aircraft, userLoc) {
